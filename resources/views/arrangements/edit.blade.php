...
<div class="container py-4">
    <h1>Edit Arrangements</h1>
    <p>Add, update, or delete arrangements below.</p>
    ...
    <!-- Add New Arrangement -->
    <form method="POST" action="{{ route('arrangements.store') }}" enctype="multipart/form-data" class="mb-4">
        @csrf
        <div class="row g-2">
            <div class="col-md-3">
                <input type="text" name="name" placeholder="Arrangement Name" required class="form-control">
            </div>
            <div class="col-md-4">
                <input type="text" name="description" placeholder="Description" required class="form-control">
            </div>
            <div class="col-md-2">
                <input type="number" name="price" placeholder="Price" required class="form-control" step="0.01">
            </div>
            <div class="col-md-2">
    ...
    <ul class="list-group">
        @foreach($arrangements as $arr)
            <li class="list-group-item d-flex justify-content-between align-items-center">
                <form method="POST" action="{{ route('arrangements.update', $arr->id) }}" enctype="multipart/form-data" class="d-flex gap-2 w-100">
                    @csrf
                    <input type="text" name="name" value="{{ $arr->name }}" class="form-control form-control-sm" required>
                    <input type="text" name="description" value="{{ $arr->description }}" class="form-control form-control-sm" required>
                    <input type="number" name="price" value="{{ $arr->price }}" class="form-control form-control-sm" step="0.01" required>
                    <input type="file" name="image" class="form-control form-control-sm">
                    <button type="submit" class="btn btn-sm btn-primary">Update</button>
                </form>

                <!-- Delete button -->
                <form action="{{ route('arrangements.destroy', $arr->id) }}" method="POST" class="ms-2">
                    @csrf
                    @method('DELETE')
